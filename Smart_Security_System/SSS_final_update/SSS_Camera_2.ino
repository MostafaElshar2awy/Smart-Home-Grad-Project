...
// LCD Configuration
// I2C LCD with address 0x27 (or 0x3F for some modules), 16x4 characters
...
void loop() {
  ...
  // Handle system modes
  if (systemLocked) {
    // Skip mode handling during lockout
    return;
  }
  if (systemMode == 0) {
    handleModeSelection();
...
void handleFingerprintMode() {
  if (systemLocked) {
    // Skip fingerprint scanning during lockout
    lcd.clear();
    lcd.print("System Locked!");
    lcd.setCursor(0, 1);
    lcd.print("Try Password Later");
    delay(1500);
    showModeSelection();
    return;
  }
...
int getFingerprintID() {
  if (systemLocked) {
    return 0; // Return no finger detected during lockout
  }
...
void showAdminMenu() {
  while (adminMode) {
    ...
    lcd.setCursor(0, 1);
    lcd.print("1:Finger 2:Pass 3:Log");
    delay(1500); // Display first line briefly
    lcd.clear();
    lcd.print("Admin Menu:");
    lcd.setCursor(0, 1);
    lcd.print("4:Clear");
...
void clearEEPROM() {
  for (int i = 0; i < logIndexAddress; i++) {
    EEPROM.update(i, 0);
  }
  logIndex = 0;
  for (int i = 0; i < maxLogs; i++) {
